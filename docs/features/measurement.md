...
計測開始ボタンの有効化条件と、エラーメッセージの優先順位：

| 優先度 | 条件 | エラーメッセージ |
|--------|------|----------------|
| 1 | 車両未登録 | ガレージから車両を登録してください |
| 2 | PRO モード ON かつ車両選択済み かつ ギア比未設定 | 車両設定からギア比を設定してください |
| 3 | PRO モード ON かつ車両選択済み かつ タイヤサイズ未設定 | 車両設定からタイヤサイズを設定してください |
| 4 | PRO モード ON かつ車両選択済み かつ ギア未選択 | 計測ギアの選択をしてください |

開始ボタン有効条件：`selectedVehicleId != null && (!proModeActive || (canSelectGear && selectedGearIndex != null))`
